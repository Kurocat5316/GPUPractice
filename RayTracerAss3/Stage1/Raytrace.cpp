...
void renderGPU(Scene scene, const int width, const int height, const int aaLevel)
{
	...
	err = clSetKernelArg(kernel, 6, sizeof(cl_mem), &clBufferOut);
	if (err != CL_SUCCESS) {
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < scene.numMaterials; i++) {
		printf("materialContainerSize[%d]: %d\n", i, scene.materialContainer[i].size);
	}
	...
	for (int i = 0; i < times; i++)
	{
		...
		renderGPU(scene, width, height, samples);
