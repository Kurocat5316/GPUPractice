...
Colour traceRay(const Scene* scene, Ray viewRay)
{
	...
	for (int level = 0; level < MAX_RAYS_CAST; ++level)
	{
		...
		if (!intersect.insideObject) output = Colour(1.0f, 1.0f, 1.0f); //+= coef * applyLighting(scene, &viewRay, &intersect);
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < times; i++)
	{
		...
		//render(&scene, width, height, samples);
